// reusable footer component on main pages
const Footer = () => {

    return (
        <footer className="footer">
            <div className="footer-copyright"> &copy; JWMP 2025 </div>
            <a id="card-hyperlink" className="footer-about-link" href="https://www.kingjamesbibleonline.org/"> e-Bible</a>
            <a id="card-hyperlink" className="footer-about-link" href="https://onlinenotepad.org/notepad"> Notepad</a>
            <a id="card-hyperlink" className="footer-about-link" href="./#/about"> About</a>
            <a id="jwmp-hyperlink" className="footer-jwmp-link" href="https://www.jwmp.solutions"> JWMP</a>
        </footer>
    )
}

export default Footer;